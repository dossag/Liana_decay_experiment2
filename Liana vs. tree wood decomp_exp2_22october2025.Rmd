...
in_dat %>% group_by(Tag) %>% filter(n() > 1)
library(tidyr)
...
https://fennerschool-associated.anu.edu.au/mensuration/BrackandWood1998/BARK.HTM#:~:text=The%20bark%20percent%20of%20a%20log%20may%20be,bark%20respectively%20at%20the%20centre%20of%20the%20log.
...
```{r,message=FALSE,warning=FALSE}
# Initial traits do not vary between the diameter class so we can instead use the average mean per species 
...
Calculate percentage mass loss percentage mass loss was calculated using the following equation: 
...
plot_small_crs<- ggplot(data = forth_harvest_small_crs, aes(x = growth_form, y =W_D_prop)) + theme_bw()+
  ...
  geom_boxplot(aes(fill = growth_form), color = "black")+ 
    annotate("text", x = 2, y = 30, size=3.5, hjust=0.5,label = as.character(expression(paste( F["(1,14)"],"   =   5.26")))) +
  annotate("text", x = 2, y = 25, size=3.5, label = "p = 0.03")+
  theme(axis.line = element_line(colour = "black", size = 0.5),
        axis.ticks.x=element_line(colour = "black"),
        axis.ticks.y=element_line(colour = "black"),
...
library(clusterSim) 
dim(dt_fb)
mburu2 <- dt_fb[!apply(dt_fb[,c( 'k',"mesh_size", "diameter_class","block", "mean_density","avg_brkwd_ratio",
...
mburu2$C_N<- mburu2$Carbon_wood/mburu2$Nitrogen_wood
mburu2$lig_N<- mburu2$ADL_wood/mburu2$Nitrogen_wood
dim(mburu2)
pcn<- ungroup(mburu2) %>% dplyr::select(Carbon_wood, Nitrogen_wood, cellulose_wood, hemicellulose_wood, ADL_wood,Tannins_wood,Calcium_wood, Potassium_wood,magesium_wood,Manganese_wood,Phosphorous_wood,sugars_wood,Silicon_wood, C_N,lig_N)
...
s$mesh_size <- mburu2$mesh_size
s$growth_form <- mburu2$growth_form
s$density <- mburu2$avg_density
s$bark_wd_ratio<- mburu2$avg_brkwd_ratio
s$species<- mburu2$species
s$k<- mburu2$k
s$block<- mburu2$block
```

# Bark pca

```{r,message=FALSE,warning=FALSE}
mburu2$C_N_bark<- mburu2$Carbon_bark/mburu2$Nitrogen_bark
mburu2$lig_N_bark<- mburu2$ADL_bark/mburu2$Nitrogen_bark

dim(mburu2)
pcnb<- ungroup(mburu2) %>% dplyr::select(Carbon_bark, Nitrogen_bark, cellulose_bark, hemicellulose_bark, ADL_bark,Tannins_bark, Calcium_bark,Potassium_bark,magesium_bark,Manganese_wood,Phosphorous_bark,sugars_wood,Silicon_bark,C_N_bark,lig_N_bark)
...
s$diameter_class<- mburu2$diameter_class
...
pca_wd <- PCA(mburu2[ ,c("growth_form","C_N","lig_N","ADL_wood", "Tannins_wood", "hemicellulose_wood", "Nitrogen_wood","Carbon_wood","cellulose_wood","Calcium_wood","Potassium_wood","magesium_wood", "Manganese_wood", "Phosphorous_wood", "sugars_wood","Silicon_bark")],scale.unit = TRUE, quali.sup = c(1:1), graph = FALSE)

summary(pca_wd)
plot.PCA(pca_wd)
# extract pc scores for first two component and add to dat dataframe
mburu2$pc1_wood <- pca_wd$ind$coord[, 1] # indexing the first column
mburu2$pc2_wood <- pca_wd$ind$coord[, 2]  # indexing the second column
pca_vars_wd <- pca_wd$var$coord %>% data.frame
pca_vars_wd $vars <- rownames(pca_vars_wd )
pca_vars_wd_m <- melt(pca_vars_wd, id.vars = "vars")

library(ggplot2)
wd_pcaplot <- ggplot(data = mburu2, aes(x = pc1_wood*-1, y = pc2_wood*-1)) +
  ...
  xlab("PC1 36.10%") + ylab("PC2 15.40%") + coord_equal() + theme_minimal() + 
...
pca_brk<- PCA(mburu2[ ,c("growth_form","C_N","lig_N","ADL_bark", "Tannins_bark", "hemicellulose_bark", "Nitrogen_bark","Carbon_bark","cellulose_bark","Calcium_bark","Potassium_bark","magesium_bark", "Manganese_bark", "Phosphorous_bark", "sugars_bark","Silicon_bark")],scale.unit = TRUE, quali.sup = c(1:1), graph = FALSE)
...
library(reshape2)
# extract pc scores for first two component and add to dat dataframe
mburu2$pc1_bark <- pca_brk$ind$coord[, 1] # indexing the first column
mburu2$pc2_bark <- pca_brk$ind$coord[, 2]  # indexing the second column
...
library(ggplot2)
brk_pcaplot <- ggplot(data = mburu2, aes(x = pc1_bark, y = pc2_bark)) +
  geom_hline(yintercept = 0, lty = 2) +
  geom_vline(xintercept = 0, lty = 2) +
  geom_point(alpha = 0.5,size= 3, aes(colour=growth_form)) +scale_shape_manual(values=c(15,19,17))+
  scale_colour_manual(values = c("lightcoral", "#00E5EE"))+
  ...
  xlab("PC1 30.0%") + ylab("PC2 21.1%") + coord_equal() + theme_minimal() + 
